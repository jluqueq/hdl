...
module pack_shell #(
  ...
  output reg [NUM_OF_CHANNELS * SAMPLES_PER_CHANNEL-1:0] mask = 'h00
);
  ...
  generate
    if (NUM_OF_CHANNELS == 1) begin
      ...
      */
      ...
      reg [NUM_OF_PORTS-2*SAMPLES_PER_CHANNEL-1:0] prev_mask = 'h00;
      reg sync = 1'b1;
...
//        localparam NUM_STAGES = i == PACK ? (PORT_ADDRESS_WIDTH - NON_POWER_OF_TWO) / 2 : (PORT_ADDRESS_WIDTH - NON_POWER_OF_TWO) % 2;
        localparam MIN_STAGE = PACK ? (i == 0 ? PORT_ADDRESS_WIDTH % 2 : 0) :
                                      (i == 0 ? NON_POWER_OF_TWO : PORT_ADDRESS_WIDTH - 1);
        localparam NUM_STAGES = PACK ? (i == 0 ? PORT_ADDRESS_WIDTH / 2 : PORT_ADDRESS_WIDTH % 2) :
                                       (i == 0 ? (PORT_ADDRESS_WIDTH - NON_POWER_OF_TWO) / 2 : (PORT_ADDRESS_WIDTH - NON_POWER_OF_TWO) % 2);
      ...
      if (NON_POWER_OF_TWO == 1 && PACK == 1) begin: gen_output_buffer
        localparam DELAYED_DATA_WIDTH = TOTAL_DATA_WIDTH - 2 * CHANNEL_DATA_WIDTH;

        reg [DELAYED_DATA_WIDTH-1:0] data_d1 = 'h00;
        reg [NUM_OF_PORTS-2*SAMPLES_PER_CHANNEL-1:0] prev_mask_d1 = 'h00;

        always @(posedge clk) begin
          if (ce_ctrl == 1'b1) begin
            prev_mask_d1 <= prev_mask;
          end
        end
        ...
        for (i = 0; i < NUM_OF_PORTS; i = i + 1) begin: gen_out_data
          localparam w = SAMPLE_DATA_WIDTH;
          localparam base = i * w;
          if (base >= DELAYED_DATA_WIDTH) begin
            assign out_data[base+:w] = data[2][base+:w];
          end else begin
            assign out_data[base+:w] = prev_mask_d1[i] == 1'b1 ? data_d1[base+:w] : data[2][base+:w];
          end
        end
      end else begin
      ...
      always @(posedge clk) begin
        if (ce_ctrl == 1'b1) begin
          {prev_mask,mask} <= (((1 << (enable_count+1)) - 1) << rotate) | prev_mask;
        end
      end
